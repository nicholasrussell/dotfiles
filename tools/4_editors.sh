...
function install_emacs_debian {
  ...
  if [ ! -d /opt/emacs ] || [[ -v DOTFILES_TOOLS_FORCE ]]; then
    ...
    sudo git clone --depth 1 --branch emacs-31 https://git.savannah.gnu.org/git/emacs.git /opt/emacs
    sudo chown -R nrussell:nrussell /opt/emacs
    cd /opt/emacs || return
    # export CC="gcc-10"
    export CFLAGS="$CFLAGS -O2"
    ./autogen.sh
    ./configure --with-imagemagick --with-json --with-mailutils --with-native-compilation=aot --with-pgtk --with-tree-sitter --with-xwidgets --with-xml2
    make -j"$(nproc)"
    sudo make install
    cd - || return
